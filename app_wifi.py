...
def load_config():
    ...
    try:
        ...
        cfg.setdefault("wifi", {})
        cfg["wifi"].setdefault("host", DEFAULT_CONFIG["wifi"]["host"])
        cfg["wifi"].setdefault("port", DEFAULT_CONFIG["wifi"]["port"])
...
# WiFi (TCP) worker
# =========================
class WifiWorker(QtCore.QObject):
    log = QtCore.Signal(str)
    detect = QtCore.Signal(str, str, str)  # side(in/out), rfid(uid), slot
    ...
    def send_line(self, s: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                # will be cleaned up by run()
                return False

    @QtCore.Slot(str, int)
    def apply_config(self, host: str, port: int):
        host = (host or "0.0.0.0").strip()
        port = int(port or 3333)
        self._host = host
        self._port = port
        self.log.emit(f"[WIFI] Config updated: listen {self._host}:{self._port} (restart app to apply)")

    def _close_conn_nolock(self):
        try:
            if self._conn:
                self._conn.close()
        except Exception:
    ...
    def _parse_lines(self, chunk: bytes):
        self._rx_buf += chunk
        while b"\n" in self._rx_buf:
    ...
    def run(self):
        while not self._stop:
            cfg = load_config()
            host = (cfg.get("wifi", {}).get("host", "0.0.0.0") or "0.0.0.0").strip()
            port = int(cfg.get("wifi", {}).get("port", 3333) or 3333)

            try:
                self.log.emit(f"[WIFI] Listening {host}:{port} ...")
                ...
                while not self._stop:
                    try:
                        ...
                        self.log.emit(f"[WIFI] Accept error: {e}")
                        break
                    ...
                    self.log.emit(f"[WIFI] ESP32 connected from {addr[0]}:{addr[1]}")
                    ...
                    while not self._stop:
                        try:
                            ...
                            self._parse_lines(data)
                        except socket.timeout:
                            continue
                        except Exception as e:
                            self.log.emit(f"[WIFI] Connection closed: {e}")
                            with self._lock:
                                self._close_conn_nolock()
                            break

            except Exception as e:
                self.log.emit(f"[WIFI] Server error: {e} (retry)")
                time.sleep(1.5)
            finally:
                with self._lock:
                    self._close_conn_nolock()
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # WiFi TCP (ESP32 will connect to this PC)
        self.ed_wifi_host = QtWidgets.QLineEdit(cfg.get("wifi", {}).get("host", "0.0.0.0"))
        self.ed_wifi_port = QtWidgets.QSpinBox()
        self.ed_wifi_port.setRange(1, 65535)
        self.ed_wifi_port.setValue(int(cfg.get("wifi", {}).get("port", 3333)))
        ...
        form.addRow("WiFi host (bind)", self.ed_wifi_host)
        form.addRow("Listen port (WiFi TCP)", self.ed_wifi_port)
    ...
    def _on_save(self):
        ...
        cfg["wifi"] = {"host": (self.ed_wifi_host.text() or "0.0.0.0").strip(), "port": int(self.ed_wifi_port.value())}
        cfg.setdefault("app", {})
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.append_log("[APP] Ready. Streaming cams. Waiting detect_in/detect_out from ESP32 over WiFi (TCP)...")
    ...
    def on_settings_saved(self, cfg: dict):
        ...
        QtCore.QMetaObject.invokeMethod(
            ...
            QtCore.Q_ARG(str, (cfg.get("wifi", {}).get("host", "0.0.0.0") or "0.0.0.0")),
            QtCore.Q_ARG(int, int(cfg.get("wifi", {}).get("port", 3333) or 3333)),
        )
